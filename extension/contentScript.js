...
const filterVideos = async (searchString) => {
    ...
    try {
        ...
        // removing function for each element (that waits for thumbnails and titlte to load)
        const processElement = async (element, prediction) => {
            ...
            try {
                if (prediction !== searchString) {
                    const thumbnail = await waitForThumbnail();
                    const titleElement = element.querySelector("#video-title");
                    
                    // Process thumbnail
                    if (thumbnail) {
                        console.log("Setting thumbnail src to cross.png");
            ...
            try {
                ...
                if (t_dash_vector) {
                    for(let i = 0; i < t_dash_vector.length; i++) {
                        if (elements[i]) {
                            //processing each element separately
                            await processElement(elements[i], t_dash_vector[i].predicted_label);
                        }
                    }
                }
            } catch (error) {
